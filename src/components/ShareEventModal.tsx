...
export function ShareEventModal({
  ...
  const checkSharedStatus = useCallback(async () => {
    if (!event || activeMemberships.length === 0) return;
    setChecking(true);
    try {
        ...
        activeMemberships.map(async (m) => {
          try {
            const data = await api.get<SharedStatus>(
              `/api/v1/communities/${encodeURIComponent(m.communityDid)}/content/check?documentUri=${encodeURIComponent(event.uri)}`,
            );
  ...
  const handleShare = async (communityDid: string) => {
    ...
    try {
      ...
      try {
        const data = await api.get<SharedStatus>(
          `/api/v1/communities/${encodeURIComponent(communityDid)}/content/check?documentUri=${encodeURIComponent(event.uri)}`,
        );
  ...
  return (
    <DialogRoot open={open} onOpenChange={() => handleClose()}>
      <DialogContent maxW={{ base: '95vw', sm: '500px' }} mx={{ base: 2, sm: 4 }}>
        <DialogHeader>
          <DialogTitle>Share Event with Community</DialogTitle>
          <DialogCloseTrigger />
        </DialogHeader>
